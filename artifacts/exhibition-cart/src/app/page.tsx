              ...
              >
                                ...
                                <tr key={sIdx} className="border-t border-slate-300">
                                  ...
                                  <td className="py-1.5">
                                    {shelf.layout_type === "none" ? (
                                      ...
                                      // Deduplicate: same item ID + same effective language => show only the first occurrence
                ...
                <div id="export-supplementary-notes" className="w-full mt-4 max-w-[820px] mx-auto">
                  {isEditingNotes ? (
                          ...
                          <div key={idx} className="flex gap-3 items-start group">
                            <div className="flex flex-col gap-1.5 pt-1">
                              {[
                                ...
                                <button
                                  ...
                                  className={`w-3.5 h-3.5 rounded-full transition-all hover:scale-125 ${c.color} ${line.color === c.val ? "ring-2 ring-offset-1 ring-slate-400 scale-110" : "opacity-40"} ${isLayoutLocked ? "cursor-not-allowed" : ""}`}
                                />
                              ))}
                            </div>
                            <div className="flex-1 relative">
                              <textarea
                                ...
                                className={`w-full text-xs text-foreground bg-slate-50/50 border border-slate-100 rounded-xl outline-none resize-none leading-relaxed font-medium p-3 focus:ring-2 focus:ring-primary/10 transition-all ${isLayoutLocked ? "opacity-70" : ""}`}
                    ...
                    <div 
                      className={`relative group cursor-pointer hover:bg-slate-50/30 p-2 rounded-xl transition-all ${isLayoutLocked ? "cursor-default" : ""}`} 
                    ...
                    <div className="space-y-3">
                       <div className="space-y-1">
                          ...
                          <input 
                            ...
                            disabled={isLayoutLocked}
                            placeholder="お名前を入力"
                            className={`w-full text-xs font-bold border border-slate-100 rounded-xl px-3 py-2 bg-slate-50 text-slate-800 outline-none focus:border-primary/30 transition-all ${isLayoutLocked ? "opacity-50 cursor-not-allowed" : ""}`}
                          />
                       </div>
                       <div className="space-y-1">
                          ...
                          <textarea 
                            ...
                            disabled={isLayoutLocked}
                            placeholder="コンセプトを入力してください"
                            className={`w-full text-xs font-bold border border-slate-100 rounded-xl px-3 py-2 bg-slate-50 text-slate-800 outline-none focus:border-primary/30 h-24 resize-none transition-all ${isLayoutLocked ? "opacity-50 cursor-not-allowed" : ""}`}
                          />
                       </div>
                    </div>
                    ...
                    <div className="space-y-3 pt-3 border-t border-slate-100">
                        <div className="flex items-center justify-between px-1">
                          ...
                          <button 
                            onClick={() => { if (!isLayoutLocked) setNotes(prev => [...prev, { text: "", color: "inherit" }]); }} 
                            disabled={isLayoutLocked}
                            className={`text-[10px] font-bold text-primary flex items-center gap-1 ${isLayoutLocked ? "opacity-50 cursor-not-allowed" : ""}`}
                          >
                                 ...
                                 <div className="flex gap-2">
                                    {[
                                      ...
                                      <button
                                        ...
                                        disabled={isLayoutLocked}
                                        onClick={() => { if (!isLayoutLocked) setNotes(prev => prev.map((l, i) => i === idx ? { ...l, color: c.val } : l)); }}
                                        className={`w-4 h-4 rounded-full ${c.color} ${line.color === c.val ? "ring-2 ring-offset-1 ring-slate-300" : "opacity-30"} ${isLayoutLocked ? "cursor-not-allowed" : ""}`}
                                      />
                                    ))}
                                 </div>
                                 <button 
                                   onClick={() => { if (!isLayoutLocked) setNotes(prev => prev.filter((_, i) => i !== idx)); }} 
                                   disabled={isLayoutLocked}
                                   className={`text-red-300 hover:text-red-500 ${isLayoutLocked ? "opacity-0 cursor-not-allowed" : ""}`}
                                 >
